...
class TestPackagistClient:
    ...
    def sample_package_data(self):
        return {
            ...
            "favers": 50000,
            "versions": {"11.x-dev": {"version": "11.x-dev", "require": {"php": ">=8.1"},
                                       "require-dev": {"mockery/mockery": "^1.6"}}},
        }

    @pytest.mark.asyncio
    async def test_get_package_info_async_success(self, client, sample_package_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value={"package": sample_package_data}):
    ...
    async def test_get_package_info_async_calls_correct_url(self, client, sample_package_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value={"package": sample_package_data}) as mock_get:
    ...
    def test_get_package_info_sync_success(self, client, sample_package_data):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock,
    ...
    async def test_search_packages_success(self, client, sample_package_data):
        with patch.object(client, '_get', new_callable=AsyncMock,
                          return_value={"results": [sample_package_data]}) as mock_get:
    ...
    async def test_get_versions_success(self, client, sample_package_data):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock,
                          return_value={"name": "laravel/laravel", "versions": sample_package_data["versions"]}):
    ...
    async def test_get_package_version_success(self, client, sample_package_data):
        with patch.object(client, '_get', new_callable=AsyncMock, return_value={"package": sample_package_data}):
