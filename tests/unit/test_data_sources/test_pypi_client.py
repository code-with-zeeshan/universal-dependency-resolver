...
class TestPyPIClient:
    ...
    async def test_get_package_info_async_success(self, client, sample_pypi_response):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_pypi_response):
    ...
    async def test_get_package_info_async_calls_correct_url(self, client, sample_pypi_response):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_pypi_response) as mock_get:
    ...
    async def test_get_package_info_async_handles_bad_data(self, client):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value={'invalid': True}):
    ...
    def test_get_package_info_sync_success(self, client, sample_pypi_response):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock, return_value=sample_pypi_response):
    ...
    def sample_processed_pypi_response(self):
        return {
            ...
            'version': '2.3.3',
            'versions': [
                {'version': '2.3.3', 'upload_time': '2023-09-01T00:00:00', 'requires_python': '>=3.8',
                 'python_versions': ['3.8', '3.9', '3.10', '3.11'], 'has_binary_wheel': True, 'has_source': True,
                 'yanked': False, 'platforms': ['linux', 'macos']},
                {'version': '2.3.2', 'upload_time': '2023-08-15T00:00:00', 'requires_python': '>=3.8',
                 'python_versions': ['3.8', '3.9', '3.10', '3.11'], 'has_binary_wheel': True, 'has_source': False,
                 'yanked': False, 'platforms': ['linux']},
            ],
    ...
    async def test_get_versions_success(self, client, sample_processed_pypi_response):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock, return_value=sample_processed_pypi_response):
    ...
    async def test_search_calls_xmlrpc(self, client):
        with patch.object(client, '_search_xmlrpc', new_callable=AsyncMock, return_value=[{'name': 'flask'}]), \
    ...
    async def test_search_falls_back_to_web_scraping(self, client):
        with patch.object(client, '_search_xmlrpc', new_callable=AsyncMock, return_value=[]), \
             patch.object(client, '_search_web_scraping', new_callable=AsyncMock, return_value=[{'name': 'flask'}]), \
             patch.object(client, '_search_fallback', new_callable=AsyncMock) as mock_fallback:
            results = await client.search('flask')
        assert len(results) == 1
        mock_fallback.assert_not_called()
    ...
    async def test_search_returns_empty_on_exception(self, client):
        with patch.object(client, '_search_xmlrpc', new_callable=AsyncMock, side_effect=Exception('XML-RPC error')):
            results = await client.search('flask')
        assert results == []
    ...
    def test_extract_python_versions_from_wheel(self, client):
        ...
        assert '3.9' in client._extract_python_versions_from_wheel('pkg-1.0-cp39-cp39-win_amd64.whl')
        assert set() == client._extract_python_versions_from_wheel('pkg-1.0.tar.gz')

    def test_extract_platform_from_wheel(self, client):
        assert client._extract_platform_from_wheel('pkg-1.0-cp39-cp39-win_amd64.whl') == 'Windows x64'
    ...
    def test_parse_keywords(self, client):
        assert client._parse_keywords('wsgi,werkzeug,flask,web') == ['wsgi', 'werkzeug', 'flask', 'web']
