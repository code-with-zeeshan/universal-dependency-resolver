...
class TestCocoaPodsClient:
    ...
    async def test_get_package_info_async_success(self, client, sample_pod_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_pod_data):
    ...
    async def test_get_package_info_async_calls_correct_url(self, client, sample_pod_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_pod_data) as mock_get:
    ...
    def test_get_package_info_sync_success(self, client, sample_pod_data):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock, return_value=sample_pod_data):
