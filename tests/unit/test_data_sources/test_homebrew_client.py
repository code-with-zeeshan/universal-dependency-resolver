...
class TestHomebrewClient:
    ...
    async def test_get_package_info_async_success(self, client, sample_formula_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_formula_data):
    ...
    async def test_get_package_info_async_calls_correct_url(self, client, sample_formula_data):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=sample_formula_data) as mock_get:
    ...
    def test_get_package_info_sync_success(self, client, sample_formula_data):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock, return_value=sample_formula_data):
            result = client.get_package_info('curl', PackageType.FORMULA)
        assert result is not None
        assert result['name'] == 'curl'

    def test_package_exists_returns_true(self, client):
        with patch.object(client, 'get_package_info_async', new_callable=AsyncMock, return_value={"name": "curl"}):
    ...
    async def test_search_packages_empty_on_no_results(self, client):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, return_value=[]):
            results = await client.search_packages('nonexistent')
        assert results == []

    @pytest.mark.asyncio
    async def test_search_packages_empty_on_exception(self, client):
        with patch.object(client, 'cached_get', new_callable=AsyncMock, side_effect=Exception('Error')):
