...
class TestExportGenerator:
    ...
    def test_initialization(self, generator):
        ...
        assert 'requirements.txt' in generator.formats
        assert 'package.json' in generator.formats
        assert 'Dockerfile' in generator.formats
    ...
    def test_generate_requirements_txt(self, generator, sample_packages, sample_system_info):
        ...
        assert '# Generated by Universal Dependency Resolver' in result
    ...
    def test_requirements_txt_format_filtering(self):
        """Test that RequirementsTxtFormat filters Python packages"""
        fmt = RequirementsTxtFormat()
        packages = [
            PackageInfo('numpy', '1.24.3', PackageEcosystem.PYPI),
            PackageInfo('react', '18.2.0', PackageEcosystem.NPM),
            PackageInfo('pandas', '2.0.1', PackageEcosystem.PYPI)
        ]

        result = fmt.generate(packages, {}, {})

        assert 'numpy==1.24.3' in result
        assert 'pandas==2.0.1' in result
        assert 'react' not in result

    def test_package_json_format_filtering(self):
        """Test that PackageJsonFormat filters NPM packages"""
        fmt = PackageJsonFormat()
        packages = [
            PackageInfo('numpy', '1.24.3', PackageEcosystem.PYPI),
            PackageInfo('react', '18.2.0', PackageEcosystem.NPM),
            PackageInfo('lodash', '4.17.21', PackageEcosystem.NPM)
        ]
    ...
    def test_generate_with_options(self, generator, sample_packages):
        ...
        assert isinstance(result, str)
        # Options should be passed through (exact behavior depends on format implementation)
