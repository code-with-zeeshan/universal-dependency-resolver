...
class TestConflictResolver:
    ...
    def test_solver_timeout_applied_and_reset(self, resolver):
        ...
        with patch.object(resolver.solver, 'set') as mock_set, \
             patch.object(resolver.solver, 'reset'):
    ...
    async def test_batch_resolution_with_errors(self, resolver):
        ...
        with patch.object(resolver, 'resolve_dependencies_async') as mock_resolve:
            mock_resolve.side_effect = [Exception("Network error"), {'status': 'success'}]
    ...
    async def test_batch_resolution_runs_batches_concurrently(self, resolver):
        ...
        with patch.object(resolver, 'resolve_dependencies_async', side_effect=fake_resolve) as mock_resolve:
            results = await asyncio.wait_for(
                resolver.resolve_batch(package_batches, system_info),
                timeout=1.0
            )

        assert start_event.is_set()
        assert mock_resolve.call_count == len(package_batches)
        assert {r['batch'] for r in results if r['status'] == 'success'} == {'batch-a', 'batch-b'}
    ...
    def test_format_solution(self, resolver):
        ...
        resolver.version_vars = {'pkg_v1': z3.Bool('pkg_v1'), 'pkg_v2': z3.Bool('pkg_v2')}
    ...
    def test_error_handling_in_resolution(self, resolver):
        ...
        with patch.object(resolver, '_build_dependency_graph', side_effect=Exception("Graph error")):
