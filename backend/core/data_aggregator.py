...
class DataAggregator:
    ...
    async def get_package_info(self, 
        ...
        for eco in ecosystems:
            tasks.append(self._fetch_package_data(
                eco, 
                package_name, 
                version,
                include_dependencies,
    ...
    async def _fetch_package_data(self, 
                                ecosystem: Ecosystem, 
                                package_name: str,
                                version: Optional[str],
                                include_dependencies: bool,
                                include_versions: bool) -> Dict:
    ...
    async def _merge_ecosystem_data(self, 
                                  aggregated: Dict, 
                                  ecosystem: Ecosystem, 
                                  data: Dict):
        ...
        if 'system_requirements' in data:
            aggregated['system_requirements'][ecosystem.value] = self._normalize_system_requirements(
    ...
    def _normalize_dependencies(self, 
                              deps: Union[Dict, List], 
                              ecosystem: Ecosystem) -> Dict[str, List[Dependency]]:
                ...
                if category in deps:
                    ...
                    if isinstance(cat_deps, dict):
                        for name, version_spec in cat_deps.items():
                            normalized['all'].append(Dependency(
                                name=name,
    ...
    def _normalize_system_requirements(self, 
                                     reqs: Union[Dict, List], 
                                     ecosystem: Ecosystem) -> List[SystemRequirement]:
        ...
        if isinstance(reqs, dict):
            ...
            if 'python' in reqs or 'python_version' in reqs:
                ...
                if isinstance(py_req, str):
                    normalized.append(SystemRequirement(
                        type='runtime',
                        name='python',
                        version_spec=py_req
                    ))
        ...
        if len(aggregated['ecosystems']) > 1:
            ...
            if len(versions_by_eco) > 1:
                ...
                if len(set(latest_versions.values())) > 1:
                    conflicts.append(CompatibilityIssue(
    ...
    def _are_version_specs_compatible(self, specs: List[str]) -> bool:
        ...
        has_exact = any(not any(op in spec for op in ['>', '<', '~', '^', '*']) for spec in unique_specs)
    ...
    def _calculate_quality_metrics(self, aggregated: Dict) -> Dict[str, float]:
        ...
        # Ecosystem coverage
        total_ecosystems = len([e for e in Ecosystem if e not in [Ecosystem.DOCS, Ecosystem.CUSTOM_DB]])
        covered_ecosystems = len([e for e in aggregated['ecosystems'] if e not in ['docs', 'custom_db']])
        metrics['ecosystem_coverage'] = covered_ecosystems / total_ecosystems if total_ecosystems > 0 else 0
    ...
    async def _aggregate_documentation(self, 
                                     package_name: str, 
                                     ecosystems: List[Ecosystem]) -> Dict[str, Any]:
        ...
        for eco in ecosystems:
            if eco == Ecosystem.PYPI:
                ...
                docs['official_docs'].append({
                    'source': 'docs.rs',
                    'url': f'https://docs.rs/{package_name}'
                })
    ...
    async def _merge_custom_data(self, aggregated: Dict, custom_data: Dict):
        ...
        if 'known_conflicts' in custom_data:
            for conflict in custom_data['known_conflicts']:
                aggregated['conflicts'].append(CompatibilityIssue(
                    severity=ConflictSeverity.HIGH,
                    ecosystem=Ecosystem.CUSTOM_DB,
    ...
    async def check_vulnerabilities(self, package_name: str, ecosystem: str, version: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(OSV_API_URL, json=query, timeout=10) as response:
    ...
    async def search_packages(self, 
        ...
        if ecosystems:
            ecosystems = [Ecosystem(sanitize_ecosystem_name(e)) if isinstance(e, str) else e for e in ecosystems]
        else:
            ecosystems = [e for e in Ecosystem if e not in [Ecosystem.DOCS, Ecosystem.CUSTOM_DB]]
    ...
    async def _search_in_ecosystem(self, 
                                 ecosystem: Ecosystem, 
                                 client: Any, 
                                 query: str, 
                                 limit: int) -> List[Dict]:
    ...
    async def _check_inter_package_conflicts(self, 
                                           packages: List[Dict], 
                                           report: Dict):
        ...
        for dep_name, requirements in all_deps.items():
            if len(requirements) > 1:
                ...
                if len(unique_specs) > 1 and not self._are_version_specs_compatible(list(unique_specs)):
...
async def example_usage():
    async with DataAggregator() as aggregator:
        ...
        compatibility = await aggregator.check_compatibility(
            packages,
            {'python': '3.9.0', 'os': 'linux', 'arch': 'x86_64'}
        )
