...
class SystemScanner:
    ...
    async def scan_all(self, categories: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if categories:
            scan_categories = {k: v for k, v in all_categories.items() 
                             if k in categories and v is not None}
        else:
    ...
    def _get_windows_version(self) -> Optional[Dict[str, str]]:
        ...
        try:
            output = subprocess.check_output(['wmic', 'os', 'get', 'Caption,Version,BuildNumber', '/value']).decode()
            info = {}
    ...
    def detect_cpu_info(self) -> Dict[str, Any]:
        ...
        if HAS_CPUINFO:
            try:
                ...
                cpu_info = CPUInfo(
                    ...
                    count_physical=info.get('count', psutil.cpu_count(logical=False) if HAS_PSUTIL else 1),
                    count_logical=psutil.cpu_count(logical=True) if HAS_PSUTIL else info.get('count', 1),
        ...
        cpu_data['virtualization'] = self._detect_virtualization_support(cpu_data.get('features', []))
    ...
    def _detect_opencl_info(self) -> Optional[Dict[str, Any]]:
        ...
        opencl_info = {'available': False, 'version': None, 'devices': []}
    ...
    def _detect_vulkan_info(self) -> Optional[Dict[str, Any]]:
        ...
        vulkan_info = {'available': False, 'version': None}
    ...
    def _detect_physical_disks(self) -> List[Dict[str, Any]]:
        ...
        if platform.system() == 'Linux':
            ...
            try:
                output = subprocess.check_output(
                    ['wmic', 'diskdrive', 'get', 'Name,Size,Model,SerialNumber', '/format:csv']
                ).decode()
    ...
    def _parse_dotnet_sdks(self, dotnet_output: str) -> List[str]:
        ...
        for line in dotnet_output.split('\n'):
            if '.NET SDKs installed:' in line:
                in_sdk_section = True
                continue
            elif in_sdk_section and line.strip() and not line[0].isspace():
                break
            elif in_sdk_section and line.strip():
    ...
    def _parse_dotnet_runtimes(self, dotnet_output: str) -> List[str]:
        ...
        for line in dotnet_output.split('\n'):
            if '.NET runtimes installed:' in line:
                ...
                continue
            elif in_runtime_section and line.strip() and not line[0].isspace():
                break
            elif in_runtime_section and line.strip():
                version_match = re.search(r'(\d+\.\d+\.\d+)', line)
                if version_match:
    ...
    def detect_installed_packages(self) -> Dict[str, List[PackageInfo]]:
        ...
        for manager, pkg_list in packages.items():
            if pkg_list:
                result[manager] = [asdict(pkg) if hasattr(pkg, '__dict__') else pkg for pkg in pkg_list]
    ...
    def _detect_firewall(self) -> Dict[str, Any]:
        ...
        firewall_info = {'enabled': False, 'type': None}
    ...
    def _check_system_updates(self) -> Dict[str, Any]:
        ...
        updates = {'available': False, 'count': 0}
        ...
        if platform.system() == 'Linux':
            ...
            # Simplified check
            updates['check_command'] = 'Check Windows Update in Settings'
    ...
    def detect_system_capabilities(self) -> Dict[str, Any]:
        ...
        dev_tools = [
            ...
            'java', 'javac', 'maven', 'gradle', 'dotnet', 'go', 'rust',
            'cargo', 'ruby', 'gem', 'perl', 'php', 'composer'
        ]
    ...
    def _check_vmware(self) -> bool:
        ...
        if platform.system() == 'Windows':
            return os.path.exists('C:\\Program Files (x86)\\VMware') or \
    ...
    def export_scan_results(self, format: str = 'json', 
                          include_sensitive: bool = False) -> Union[str, Dict]:
            ...
            if 'installed_packages' in data:
                ...
                for manager in data['installed_packages']:
                    if len(data['installed_packages'][manager]) > 20:
                        data['installed_packages'][manager] = data['installed_packages'][manager][:20]
    ...
    def _generate_summary(self, data: Dict) -> str:
        ...
        if 'platform' in data:
            ...
            if platform_info.get('distribution'):
                ...
                summary.append(f"Distribution: {dist.get('name', 'Unknown')} {dist.get('version', '')}")
        ...
        if 'cpu' in data:
            ...
            summary.append(f"Cores: {cpu.get('count_physical', 'Unknown')} physical, {cpu.get('count_logical', 'Unknown')} logical")
        ...
        if 'memory' in data:
            ...
            if mem.get('total'):
                ...
                summary.append(f"        {mem.get('available', 0) / (1024**3):.2f} GB available ({mem.get('percent', 0):.1f}% used)")
        ...
        summary.append("")
        summary.append(f"Scan completed at: {data.get('scan_metadata', {}).get('timestamp', 'Unknown')}")
...
async def example_usage():
    ...
    async with scanner:
        ...
        # Export as JSON
        json_output = scanner.export_scan_results(format='json', include_sensitive=False)
