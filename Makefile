...
test-all:                          ## Run all unit tests with coverage
	python -m pytest tests/unit/ -v --tb=short --cov=backend.core --cov=backend.api --cov=backend.cli --cov=backend.settings --cov=backend.manifest_detector --cov=backend.data_sources --cov-report=term-missing --cov-fail-under=55
