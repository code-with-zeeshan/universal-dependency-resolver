...
- **`udr update --all`** (`backend/cli/commands/update.py`): bulk-update every package in the lock file to its latest compatible version (resolution hash recomputed for each). `--dry-run` previews without writing.
...
- **Optional dependency groups excluded by default** (`backend/cli/commands/lock.py`): pyproject `optional-dependencies`, setup.py extras, and npm `optionalDependencies` are now excluded from resolution unless `--with-dev` is passed, so out-of-the-box parses match pip's base-install semantics. `--without-optional` forces exclusion.
- **AutoSolver crash isolation** (`backend/core/auto_solver.py`): a crashing solver backend (e.g. pubgrub-py `ValueError` on unparseable constraints) no longer kills the whole resolution — it is treated as a failed candidate and the next solver in the chain is tried.
...
- **`udr auth` API-key creation crashed against an empty database** (`backend/cli/commands/auth.py`): `_get_db` never ran `init_db()`, and `APIKey` requires a `user_id` — `udr auth create` failed with a foreign-key constraint on a fresh install. The DB is now initialized on access and a `cli` user is auto-created as the key owner.
- **Mixed-case package names skipped by `_upgrade_to_latest`** (`backend/core/conflict_resolver.py`): the post-solve upgrade pass keyed on display names while `_candidate_lists` is keyed by normalized (lowercased) names, so packages whose name contains uppercase (e.g. `A` in old locks) were silently skipped and kept stale versions. Names are now mapped back through `_name_map`.
- **Lock detail lookup ignored cross-ecosystem duplicates** (`backend/cli/commands/lock.py`): `package_details` is keyed by `pkg@eco` but was looked up by bare name, dropping vulnerability/integrity metadata for packages that share a name across ecosystems. Lookup now falls back to the `{name}@{ecosystem}` key.
- **PyPI license detection added** (`backend/data_sources/pypi_client.py`): `license` metadata is now extracted from package classifiers (e.g. `License :: OSI Approved :: MIT License`) when no explicit `license` field is present, so `udr check --license` covers more packages.
- **Docs refreshed for v1.4.2** (README, README_PYPI, docs/CLI.md, docs/API.md, docs/ARCHITECTURE.md, docs/COMPONENTS.md, docs/USER_GUIDE.md, docs/SDK_ROADMAP.md, docs/FAQ.md, docs/DEVELOPMENT.md, docs/TROUBLESHOOTING.md, docs/ROADMAP.md, docs/index.md, Readiness.md): 26 CLI commands, 4374 tests (3803 unit + 96 integration + 392 e2e + 83 others), graph/update/outdated/check sections rewritten, `--from-lock`/`--all`/combined-check JSON shapes, index-hosting workflow, new `docs/GOLDEN_MATRIX.md` linked from README.
